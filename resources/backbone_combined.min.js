...
options=_.extend(options,{async:false});return this.destroy(options);};eval('Backbone.Model.prototype._baseSave = '+Backbone.Model.prototype.save.toString());eval('Backbone.Model.prototype._destroy = '+Backbone.Model.prototype.destroy.toString());Backbone.Model.prototype._save=function(key,val,options){if(key==null||typeof key==='object'){attrs=key;options=val;}else{(attrs={})[key]=val;}
options=_.extend({validate:true},options);this._baseSave(attrs,_.extend({},{originalOptions:_.clone(options),originalSuccess:options.success,success:function(model,response,options){model._origAttributes=_.clone(model.attributes);if(options.originalSuccess!=undefined){options.originalSuccess(model,response,options.originalOptions);}}}));};Backbone.Model.prototype.save=function(key,val,options){this._save(key,val,options);};Backbone.Model.prototype.changedAttributes=function(diff){this._origAttributes=(this._origAttributes==undefined?{}:this._origAttributes);if(!diff)return this.hasChanged()?_.clone(this.changed):false;var val,changed=false;for(var attr in diff){if(_.isEqual(this._origAttributes[attr],(val=diff[attr])))continue;(changed||(changed={}))[attr]=val;}
return changed;};Backbone.View.prototype.AdditionalRenderCalls=[];Backbone.View.prototype.initialize=function(options){if(this.model!=undefined){if(this.model.on!=undefined){this.model.on('change',this.render,this);}}else if(this.collection!=undefined){if(this.collection.on!=undefined){this.collection.on('reset',this.render,this);this.collection.on('sync',this.render,this);this.collection.on('add',this.render,this);this.collection.on('remove',this.render,this);}}
