...
namespace Org.Reddragonit.BackBoneDotNet
{
	...
	internal class JSON
	{
        ...
        protected object ParseString(char[] json, ref int index)
        {
            string s = "";
            char c;
        ...
        protected object ParseNumber(char[] json, ref int index)
        {
            ...
            index = lastIndex + 1;
            string st = new string(numberCharArray);
            if (st.Contains("."))
                return OptimizeNumber(Double.Parse(st, CultureInfo.InvariantCulture));
            else if (st.StartsWith("-"))
                return OptimizeNumber(long.Parse(st, CultureInfo.InvariantCulture));
            else
                return OptimizeNumber(ulong.Parse(st, CultureInfo.InvariantCulture));
        }

        protected object OptimizeNumber(double val)
        {
            if (val > (double)float.MinValue && val < (double)float.MaxValue)
                return (float)val;
            return val;
        }
        ...
        protected bool SerializeValue(object value, StringBuilder builder)
        {
            ...
            {
                SerializeString(Convert.ToBase64String((byte[])value), builder);
            }
            ...
            {
                SerializeString(((Type)value).FullName, builder);
            }
            else if (value.GetType().IsSubclassOf(typeof(Exception)))
            {
                Hashtable tmp = new Hashtable();
                Exception ex = (Exception)value;
                tmp.Add("Message", ex.Message);
                tmp.Add("StackTrace", ex.StackTrace);
                tmp.Add("Source", ex.Source);
                SerializeObject(tmp, builder);
            }
            else
            {
