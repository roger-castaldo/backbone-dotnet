...
return this._set(attrs,options);};Backbone.Model.prototype.toJSON=function(){var attrs={};this._changedFields=(this._changedFields==undefined?[]:this._changedFields);for(var k in this.attributes){if(this._changedFields.indexOf(k)>=0||this.isNew()){attrs[k]=this.attributes[k];}}
return attrs;};Backbone.Model.prototype.syncSave=function(attrs,options){if(!options){options={};}
options=_.extend(options,{async:false});return this.save(attrs,options);};Backbone.Model.prototype.syncDestroy=function(options){if(!options){options={};}
options=_.extend(options,{async:false});return this.destroy(options);};Backbone.Model.prototype.destroy=function(options){options=options?_.clone(options):{};var model=this;var success=options.success;var destroy=function(){model.trigger('destroy',model,model.collection,options);};options.success=function(resp){if(resp){if(options.wait||model.isNew())destroy();if(success)success(model,resp,options);if(!model.isNew())model.trigger('sync',model,resp,options);}else if(options.error){options.error(model,response,options);}};if(this.isNew()){options.success();return false;}
...
this.wrapError(this,options);var xhr=this.sync('delete',this,options);if(!options.wait)destroy();return xhr;};eval('var wrapError = function (model, options) { \
    ...
    Backbone.Model.prototype._origSave = '+Backbone.Model.prototype.save.toString());eval('var wrapError = function (model, options) { \
        var error = options.error; \
        options.error = function (resp) { \
            if (error) error(model, resp, options); \
            model.trigger(\'error\', model, resp, options); \
    ...
    Backbone.Model.prototype._destroy = '+Backbone.Model.prototype.destroy.toString());Backbone.Model.prototype._save=function(key,val,options){Backbone.Model.prototype._baseSave(key,val,options);};Backbone.Model.prototype._baseSave=function(key,val,options){if(key==null||typeof key==='object'){attrs=key;options=val;}else{(attrs={})[key]=val;}
...
var newOptions=_.extend(_.clone(options),{originalOptions:options,originalSuccess:options.success,originalError:options.error,success:function(model,response,options){model._changedFields=[];if(options.originalSuccess!=undefined){options.originalSuccess(model,response,options.originalOptions);}},error:function(model,xhr,options){if(options.originalError!=undefined){options.originalError(model,xhr,options.originalOptions);}}});this._origSave(attrs,newOptions);};Backbone.Model.prototype.save=function(key,val,options){this._save(key,val,options);};Backbone.View.prototype.AdditionalRenderCalls=[];Backbone.View.prototype.initialize=function(options){if(this.model!=undefined){if(this.model.on!=undefined){this.model.on('change',this.render,this);}}else if(this.collection!=undefined){if(this.collection.on!=undefined){this.collection.on('reset',this.render,this);this.collection.on('sync',this.render,this);this.collection.on('sort',this.render,this);if(this.AddModel!=undefined){this.collection.on('add',this.AddModel,this);}
if(this.RemoveModel!=undefined){this.collection.on('remove',this.RemoveModel,this);}}}
_.extend(this,_.omit(options,_.keys(this)));for(var x=0;x<this.AdditionalRenderCalls.length;x++){this.on('pre_render_complete',this.AdditionalRenderCalls[x],this);}};Backbone.Collection.prototype.initialize=function(){this.isNew=true;};eval('var wrapError = function (model, options) { \
        var error = options.error; \
        options.error = function (resp) { \
            if (error) error(model, resp, options); \
            model.trigger(\'error\', model, resp, options); \
