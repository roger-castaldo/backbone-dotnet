...
acceptKeys=(acceptKeys==undefined?[]:acceptKeys);if(!_.isObject(obj))return obj;var ret;if(_.isArray(obj)){ret=[];for(var x=0;x<obj.length;x++){ret[x]=_.deepClone(obj[x],acceptKeys);}}else{switch(toString.call(obj)){case'[object String]':case'[object Date]':case'[object Function]':case'[object RegExp]':ret=obj;break;default:if(obj.clone!=undefined){ret=obj.clone();}else{ret={};for(var x=0;x<acceptKeys.length;x++){ret[acceptKeys[x]]=_.deepClone(obj[acceptKeys[x]],acceptKeys);}}
break;}}
return ret;};
