...
var newOptions=_.extend(_.deepClone(options),{originalOptions:options,originalSuccess:options.success,originalError:options.error,success:function(model,response,options){model._changedFields=[];if(options.originalSuccess!=undefined){options.originalSuccess(model,response,options.originalOptions);}},error:function(model,xhr,options){if(options.originalError!=undefined){options.originalError(model,xhr,options.originalOptions);}}});this._origSave(attrs,newOptions);};Backbone.Model.prototype.save=function(key,val,options){this._save(key,val,options);};Backbone.View.prototype.AdditionalRenderCalls=[];Backbone.View.prototype.initialize=function(options){if(this.model!=undefined){if(this.model.on!=undefined){this.model.on('change',this.render,this);}}else if(this.collection!=undefined){if(this.collection.on!=undefined){this.collection.on('reset',this.render,this);this.collection.on('sync',this.render,this);this.collection.on('add',this.render,this);this.collection.on('remove',this.render,this);this.collection.on('sort',this.render,this);}}
_.extend(this,_.omit(options,_.keys(this)));for(var x=0;x<this.AdditionalRenderCalls.length;x++){this.on('pre_render_complete',this.AdditionalRenderCalls[x],this);}};Backbone.Collection.prototype.initialize=function(){this.isNew=true;}
eval('Backbone.Collection.prototype._fetch = '+Backbone.Collection.prototype.fetch.toString());Backbone.Collection.prototype.fetch=function(options){options=options?_.clone(options):{};options.sort=false;if(this.isNew){var newOptions=_.extend(_.deepClone(options),{reset:true,silent:true,originalOptions:options,originalSuccess:options.success,originalError:options.error,success:function(collection,response,options){collection.isNew=false;if(options.originalSuccess!=undefined){options.originalSuccess(collection,response,options.originalOptions);}},error:function(collection,xhr,options){if(options.originalError!=undefined){options.originalError(model,xhr,options.originalOptions);}}});this._fetch(newOptions);}else{this._fetch(options);}};}).call(this);